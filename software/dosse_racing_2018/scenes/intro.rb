...
class IntroScene
  ...
  def draw_logo
    image = Assets::LOGO
    image_scale = MONITOR_RESOLUTION.x / image.width + ncos(@time * 0.0002) * 0.4
    image_pos = SCREEN_CENTER + Vector.from_angle(@time*0.011, 50)
  ...
  def draw_star_image
    image = Assets::STARS
    image_scale = (MONITOR_RESOLUTION.y * 2.3 / image.height) + ncos(@time * 0.00011) * 0.7
    image_pos = SCREEN_CENTER + Vector.from_angle(@time * -0.007, 80)

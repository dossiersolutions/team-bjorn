...
import static no.dossier.thatbuttonserver.ConfigReader.readConfig;
...
public final class ConfigFileWatcher implements Runnable {
    ...
    public void run() {
        try {
            ...
            Path configFilePath = directoryPath.resolve(ConfigReader.CONFIG_FILE_NAME);
            while (!stopFlag.get()) {
                ...
                if (key != null) {
                    for (WatchEvent<?> watchEvent : key.pollEvents()) {
                        ...
                        if (kind.equals(StandardWatchEventKinds.ENTRY_MODIFY)) {
                            ...
                            if (eventFilePath.equals(configFilePath)) {
